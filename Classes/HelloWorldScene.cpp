...
typedef struct _Controller{
	...
	std::function<BasicScene*()> callback;
} Controller;
...
bool HelloWorld::init()
{
    ...
    auto menuLabel = Label::createWithTTF(ttfConfig, g_aTestNames[0].test_name);       
    ...
    menuItem->setPosition(Vec2(origin.x + visibleSize.width - menuItem->getContentSize().width/2,
                               origin.y + closeItem->getContentSize().height + menuItem->getContentSize().height/2)
                          );
    menu->addChild(menuItem,1);
    return true;
}
...
{
    auto scene = g_aTestNames[0].callback();
